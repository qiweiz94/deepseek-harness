...
**Resolution in `d4530b8ebb` (same session):** GAP-1 is closed — the verbatim-tail caps no longer break the backward walk before the `retainTokens` floor is met, so token-cheap sessions keep at least the configured residual verbatim (the caps and the `nodeStepTurns` helper are removed, and a cheap-step floor regression test was added). GAP-2's O(n²) truncation is replaced by a single O(n) code-point scan plus one library truncation, with a mixed astral/ascii unit-precise regression test; the once-per-revision full-objective admission remains a deliberate cache-economy tradeoff, mitigated by the directive to read the current goal. The four NITs stand as cosmetic follow-ups.
## Success bar
