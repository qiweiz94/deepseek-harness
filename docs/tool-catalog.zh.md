...
| `@deepseek-ai/dsh-plugin-diagnostic-sifter` | `run_diagnostic_check` | `ctx.tools`、`ctx.subprocess` | `tool/call`、`tool/result` | - | run_diagnostic_check 运行仓库类型检查或范围化 vitest 套件，抑制下游导入级联与通过噪音，返回有界的根因列表及被抑制级联计数。 |
...
解析目录下的每个 TypeScript（.ts 或 .tsx）文件并列出每个文件的顶层声明——函数、类、接口、类型别名和枚举——附带 1 起始的行号范围，以及每个符号体内声明的声明与方法。忽略隐藏条目与 node_modules。在阅读大型目录树之前用它定位。路径必须存在且必须是目录。
...
解析本地 TypeScript（.ts 或 .tsx）文件并列出其顶层声明——函数、类、接口、类型别名和枚举——附带 1 起始的行号范围，以及每个符号体内声明的声明与方法。在阅读大型文件之前用它定位。路径必须存在且能无语法错误地解析。
...
运行仓库类型检查或范围化测试，并只返回根因诊断：下游模块解析级联错误被抑制并计数，重复诊断被合并，测试输出只保留失败断言细节。输出无法解析的失败运行会连同原始输出报告为解析失败——绝不会当作干净结果。
...
来源：[`packages/plugins/plugin-diagnostic-sifter/src/index.ts`](../packages/plugins/plugin-diagnostic-sifter/src/index.ts)

run_diagnostic_check 运行仓库类型检查或范围化 vitest 套件，抑制下游导入级联与通过噪音，返回有界的根因列表及被抑制级联计数。
...
在隔离的 git worktree（一个可丢弃的试运行）中运行命令，并返回试运行的 git diff 与命令的退出状态。worktree 从当前分支 detached 并在调用后移除，因此试运行不会改动主工作树。在把改动提交到真实工作树之前，用它安全地做实验（例如子代理试运行）。
...
来源：[`packages/plugins/plugin-worktree-sandbox/src/index.ts`](../packages/plugins/plugin-worktree-sandbox/src/index.ts)

sandbox_exec 在隔离的 detached git worktree 中运行命令，并返回有界的结构化 diff 与退出状态；调用后移除该 worktree。
