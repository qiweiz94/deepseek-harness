...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-plugin-ast-context` | `get_file_outline` | `ctx.tools` | `tool/call`、`tool/result` | - | get_file_outline 读取仓库相对路径的源文件并返回其顶层 TypeScript 符号；解析失败或文件缺失以错误结果呈现，而不是部分大纲。 |
