...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    await dialog.getByRole('button', { name: 'Workspace Write' }).waitFor({ timeout: scaledTimeout(10_000) })
    await expect.poll(() => dialog.getByText('语言', { exact: true }).count(), { timeout: scaledTimeout(5_000) }).toBe(1)
    await expect.poll(() => dialog.getByText('外观', { exact: true }).count(), { timeout: scaledTimeout(5_000) }).toBe(1)
    const openDocument = dialog.getByRole('button', { name: '打开配置文件' })
    await openDocument.waitFor({ timeout: scaledTimeout(10_000) })
    let openRequests = 0
    ...
    await expect.poll(() => openRequests, { timeout: scaledTimeout(5_000) }).toBe(1)
    await expect.poll(() => openDocument.isEnabled(), { timeout: scaledTimeout(5_000) }).toBe(true)
    ...
    await expect.poll(() => dialog.getByRole('button', { name: '模型' }).getAttribute('aria-current'), { timeout: scaledTimeout(5_000) }).toBe('true')
    ...
    await dialog.getByRole('heading', { name: '插件', exact: true }).waitFor({ timeout: scaledTimeout(10_000) })
    await dialog.getByRole('tab', { name: '插件列表', exact: true }).click()
    const pluginRow = dialog.locator(PLUGIN_ROW_SELECTOR)
    await pluginRow.waitFor({ timeout: scaledTimeout(10_000) })
  ...
  it('stores Permission as the default for future sessions without changing an existing session', async () => {
    ...
    await selector.waitFor({ timeout: scaledTimeout(10_000) })
    await expect.poll(() => selector.isEnabled(), { timeout: scaledTimeout(5_000) }).toBe(true)
    await selector.click()
    await page.getByRole('menuitem', { name: 'Read Only' }).click()
    await dialog.getByRole('button', { name: 'Read Only' }).waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await enable.click()
    await dialog.getByRole('button', { name: 'Full access' }).waitFor({ timeout: scaledTimeout(10_000) })
  ...
  it('uses the persisted dark preference while plugins are still loading', async () => {
    ...
    await expect.poll(() => darkCube.getAttribute('aria-pressed'), { timeout: scaledTimeout(5_000) }).toBe('true')
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: scaledTimeout(5_000) })
      .toMatch(/ui-theme:\n\s+preference: dark/)
    await page.keyboard.press('Escape')
    ...
    try {
      ...
      await loading.waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await expect.poll(() => systemCube.getAttribute('aria-pressed'), { timeout: scaledTimeout(5_000) }).toBe('true')
  ...
  it('flips the theme through the Appearance cubes and persists across reload and a distinct port', async () => {
    ...
    acknowledgeReloadConnectionLoss(tripwire, warningStart)
    await page.emulateMedia({ colorScheme: 'light' })
    await expect.poll(async () => (await readState()).attr, { timeout: scaledTimeout(5_000) }).toBe(true)
    ...
    try {
      ...
      await secondPage.waitForSelector('[class*="frame"]', { timeout: scaledTimeout(30_000) })
      await expect.poll(async () => (await readState(secondPage)).attr, { timeout: scaledTimeout(5_000) }).toBe(true)
    ...
    await systemCube.click()
    await expect.poll(() => systemCube.getAttribute('aria-pressed'), { timeout: scaledTimeout(5_000) }).toBe('true')
    await expect.poll(async () => (await readState()).attr, { timeout: scaledTimeout(5_000) }).toBe(false)
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    await dialog.getByRole('button', { name: '插话发送' }).waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await reloaded.getByRole('button', { name: '插话发送' }).waitFor({ timeout: scaledTimeout(10_000) })
    ...
    try {
      ...
      await secondPage.getByRole('dialog', { name: '设置' })
        .getByRole('button', { name: '插话发送' }).waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await reloaded.getByRole('button', { name: '排队发送' }).waitFor({ timeout: scaledTimeout(10_000) })
    expect(await page.evaluate(() => localStorage.getItem('dsh.conversation.busyEnter'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: scaledTimeout(5_000) })
      .toMatch(/ui-conversation:\n\s+busyEnter: queue/)
    await page.keyboard.press('Escape')
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    await zhDialog.waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await enDialog.waitFor({ timeout: scaledTimeout(10_000) })
    expect(await enDialog.getByRole('button', { name: 'General' }).getAttribute('aria-current')).toBe('true')
    await expect.poll(() => enDialog.getByText('Appearance', { exact: true }).count(), { timeout: scaledTimeout(5_000) }).toBe(1)
    ...
    await enTrigger.waitFor({ timeout: scaledTimeout(10_000) })
    ...
    try {
      ...
      await secondPage.getByRole('dialog', { name: 'Settings' })
        .getByRole('button', { name: 'English' }).waitFor({ timeout: scaledTimeout(10_000) })
    ...
    await page.getByRole('dialog', { name: '设置' }).waitFor({ timeout: scaledTimeout(10_000) })
    expect(await page.evaluate(() => localStorage.getItem('dsh.locale'))).toBeNull()
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: scaledTimeout(5_000) })
      .toMatch(/locale:\n\s+preference: zh/)
    await page.keyboard.press('Escape')
  ...
  it('opens an English browser in English without any stored preference', async () => {
    ...
    try {
      ...
      await enPage.waitForSelector('[class*="frame"]', { timeout: scaledTimeout(30_000) })
      ...
      await dialog.waitFor({ timeout: scaledTimeout(10_000) })
      await dialog.getByRole('button', { name: 'English' }).waitFor({ timeout: scaledTimeout(10_000) })
