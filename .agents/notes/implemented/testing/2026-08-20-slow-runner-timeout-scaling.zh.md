...
- CI 通道对每个测试和每个持久化等待容忍 4 倍（coverage）或 8 倍（consumers）墙钟时间；非法因子会在 `vitest.shared.ts`/`tools-catalog.spec.ts` 的配置求值处，或 harness 场景发起的第一次未缩放等待处大声失败，而不是静默地不缩放运行。
