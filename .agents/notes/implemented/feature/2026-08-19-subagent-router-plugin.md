...
**Routing is deterministic policy.** `Config.providers` is the ordered default candidate list; `Config.routes` provides label-routed overrides matched case-insensitively against the task `description` (first match wins). At call time the router walks the candidates, skips unregistered providers, and dispatches to the first whose `SubagentCapabilities` satisfy the request's needs — `persona`/`toolFilter`/`depthLimit` required only when the corresponding config option is set. No capable candidate fails loud with the candidates tried and the missing capabilities.
