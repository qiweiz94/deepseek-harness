...
## Follow-up: observe mode and per-turn re-arm (2026-08-20)

A parallel implementation shipped an observe-only governor; rather than a second package, its non-cancelling behavior folds in as a `mode: 'enforce' | 'observe'` config field (default `enforce`) — `observe` reports a crossed ceiling without cancelling, for measuring how often ceilings would trip before letting the governor intervene. Both modes inject the same parent report (worded for whether the run was stopped). Separately, a tripped run now reports at most once per turn and re-arms its per-turn detectors on the next `turn/start`, closing the gap where a cancelled continuable child would then run ungoverned on its parent's follow-up: the token ceiling, being cumulative, re-trips at once if the run is still over, while the failure and churn counters reset for the fresh turn. No shipped composition creates a continuable governed child yet (fork binds one-shot — the `fork-continuable-prefix-reuse` TODO), so the re-arm's active branch is forward-looking and carries a `v8 ignore` with that reason until such a composition ships.
