...
## 后续：observe 模式与按转轮重新武装（2026-08-20）

一个并行实现提供了仅观察的治理器；与其做第二个包，不如把其非取消行为折叠为 `mode: 'enforce' | 'observe'` 配置字段（默认 `enforce`）——`observe` 报告被越过的上限但不取消，用于在让治理器介入之前测量上限触发频率。两种模式都向父级注入同一份报告（措辞取决于运行是否被停止）。此外，触发的运行现在每次转轮最多报告一次，并在下一次 `turn/start` 重新武装其按转轮的检测器，堵住"被取消的可继续子代理随后在父级跟进中不受治理运行"的缺口：token 上限是累计的，若仍超限会立即再次触发，而失败与 churn 计数器为新转轮重置。目前尚无已上线组合创建可继续的受治理子代理（fork 绑定一次性——`fork-continuable-prefix-reuse` TODO），故重新武装的活动分支是前瞻性的，在此类组合上线前带有相应理由的 `v8 ignore`。
