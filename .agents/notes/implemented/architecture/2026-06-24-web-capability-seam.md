...
The `web_fetch` implementation is an anonymous public HTTP(S) fetch provider, `http`. It fetches bytes from a concrete URL, applies the basic transport hygiene below (http/https-only, credential rejection, byte/time caps, cross-origin redirect blocking, private-network blocking) and returns only the minimal model-useful result: final URL, status code, body, and truncation. It carries no browser cookies, editor credentials, git credentials, internal auth tokens, or implicit access to private services. (Pinning the validated destination address into the actual connection, to fully defeat DNS rebinding between validation and connect, is deferred — see [Deferred work](#deferred-work) and the [private-network blocking Agent Note](../feature/2026-08-20-web-fetch-http-private-network-blocking.md).)
...
- A private, loopback, link-local, carrier-grade-NAT, or otherwise non-globally-routable destination is refused (`WEB_BLOCKED_PRIVATE_NETWORK`): a literal IP hostname is checked directly, a DNS name is resolved and every returned address checked, and the check re-runs on each redirect target — not just the initial URL — so a same-origin hostname that rebinds mid-chain is still caught. On by default; a deployment that deliberately needs this provider to reach internal targets disables it through `Config.blockPrivateNetworks`. The validated address is not pinned into the connection that `fetch()` then makes, so a rebind between this check and that connection is not defended against — see [Deferred work](#deferred-work) and the [private-network blocking Agent Note](../feature/2026-08-20-web-fetch-http-private-network-blocking.md).
...
**Fetch is a network boundary, not just a read-only tool.** `web_fetch` can reach sensitive network targets or exfiltrate data through URLs. Transport hygiene (http/https-only, credential rejection, byte/time caps, cross-origin redirect blocking) and private-network blocking (on by default, config-switchable) both ship; pinning the validated address into the connection to fully defeat DNS rebinding remains deferred (see [Deferred work](#deferred-work)), so a deployment relying on this provider for SSRF defense should still treat that narrow rebind window as residual risk.
...
- Pinning `web_fetch`'s validated destination address into the actual connection: the private-network check ([private-network blocking Agent Note](../feature/2026-08-20-web-fetch-http-private-network-blocking.md)) resolves and validates a hostname before each request and redirect hop, but the connection `fetch()` then makes re-resolves DNS independently, so a name whose records change between validation and connect (DNS rebinding) is not defended against. Closing this needs the resolved, validated address connected to directly — through a custom `fetch()` dispatcher/agent `lookup`, or an equivalent connect-time override — while still presenting the original hostname for TLS SNI/certificate validation and the `Host` header.
