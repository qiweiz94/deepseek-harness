"""Drive the real ``get_file_outline`` tool through the SDK and a keyless mock model.

The model endpoint is a local mock: its first completion answers with a
``tool_calls`` delta for ``get_file_outline``, and the second (after the agent
...
output for the requested repository file.

Requires ``pnpm install`` but no build. Not collected by pytest itself; the
collected ``tests/test_file_outline_example.py`` drives the same function.
Run directly with: ``python examples/file-outline/outline.py``
"""
...
OUTLINE_PATH = "packages/plugins/plugin-ast-context/src/extractor.ts"
    ...
    def do_POST(self) -> None:
        ...
        if len(self.requests) == 1:
            self.wfile.write(
                ...
                b'"function":{"name":"get_file_outline","arguments":""}}]}}]}\n\n'
            )
            ...
            self.wfile.write(
                ...
                b'"The tool returned the outline of the extractor file."},"finish_reason":"stop"}],'
...
def run_outline(repo_root: Path, keep_sessions: bool) -> dict[str, Any]:
    session_root = Path(tempfile.mkdtemp(prefix="dsh-outline-sessions-"))
    ...
    thread = threading.Thread(target=server.serve_forever, name="mock-outline-model", daemon=True)
    thread.start()
    ...
    try:
        with DeepSeekHarness(
            ...
            env={
                ...
                "DEEPSEEK_API_KEY": "outline-demo-key",
            },
            request_timeout_seconds=30,
            shutdown_timeout_seconds=2,
        ) as harness:
            result = harness.run(
                f"Outline the file {OUTLINE_PATH} and report the symbol names.",
                session_id="outline-demo-main",
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Drive the real get_file_outline tool through the SDK against a keyless mock model."
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
