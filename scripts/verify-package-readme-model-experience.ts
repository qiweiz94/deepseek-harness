...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/lsp/lsp-typescript-inprocess': { kind: 'indirect', reason: 'The in-process TypeScript provider backend delegates model rendering to dsh-tool-lsp.' },
  ...
  'packages/session/session-retention': { kind: 'none', reason: 'The seam deletes durable data across stores for a session-management caller; v1 exposes no model-facing tool and registers nothing model-facing.' },
