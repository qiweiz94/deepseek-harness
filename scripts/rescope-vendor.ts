...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // The dynamic-plugin-activation feature emits its own `cordis/<event>` event
  // scope (`ctx.emit`/`ctx.on`/`$on`) and an unrelated `cordis` i18n namespace
  // (`PropsLocale`/`NS`/`t()`) — both spellings predate and are independent of
  // the vendored package name, so the token rule's package-subpath heuristic
  // (`'name/subpath'`) misreads them as `cordis` imports. No file in this group
  // has a genuine bare `cordis` package reference for the token rule to catch.
