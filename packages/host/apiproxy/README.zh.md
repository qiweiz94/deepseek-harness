...
- **仅 `session.search` 中位于提供方边界的失败会被脱敏**：`sessionQuery.searchSessions` 的拒绝，除了其两个会自适应重试的代码（`SESSION_QUERY_INVALID_LIMIT`、`SESSION_QUERY_STALE_CURSOR`）以及消息固定且安全的 `SESSION_QUERY_ABORTED` 之外，都会在服务端完整记录日志，并以通用的 `session search provider failed` 作答，因为一次索引损坏或持久化失败可能携带原始的后端细节（一段 SQLite 错误字符串、一个索引文件路径）。`session.search` 中此网关自身的协议守卫诊断信息（提供方调用的工作预算、超量的一页、重复的续读游标）不属于提供方细节，仍会原样出现在线路上；本文件中的其他每个方法目前仍以 `code: 'internal'` 搭配未脱敏的 `String(error)` 作答——该网关仍是单用户本地服务，若要将其扩展为共享部署，需要先对那些其他方法做同样的处理。
