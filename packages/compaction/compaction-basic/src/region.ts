 ...
 * verbatim. The tail keeps at least `retainTokens` tokens (the operator's
 * post-compaction residual floor), walking over as many recent steps and turns
 * as needed to reach that floor, so a run of token-cheap steps cannot silently
 * under-deliver the configured residual envelope. The compacted span shadows
 * everything before it, never a tool-call/result pair.
...
export function selectCompactableRange(
  ...
  for (let index = pricedNodes.length - 1; index >= 0; index -= 1) {
    ...
    // The residual floor is authoritative: keep backing the cut over any number
    // of recent steps/turns until at least `retainTokens` are retained verbatim.
    keepFromIdx = index
