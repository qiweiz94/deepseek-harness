...
零依赖的原子、持久文件替换，供绝不允许在磁盘上留下不完整、被符号链接劫持或权限过宽内容的文件型存储共用：用户设置文档（`dsh-settings-file`）与凭据存储（`dsh-credentials-local`）。
...
- **临时文件在 rename 之前会被 fsync**：同步失败会中止写入（移除临时文件并重新抛出该失败），而不是冒险发布设备尚未收到的内容，因此崩溃绝不会让空内容或部分写入的内容变成正式内容。
- **rename 之后会尽力对父目录做 fsync**：该步骤失败并不会让本次调用失败——rename 已经把新内容提交到最终文件名下——但会缩小崩溃后目录项可能未被观察到的窗口。参见下方“已知限制与暂缓事项”。
- 自动创建父目录；在 rename 提交之前发生的任何失败都会移除临时文件并重新抛出该失败；读取方只会观察到旧内容或完整的新内容。
...
- **目录项持久性是尽力而为，不作保证**——临时文件的内容会在 rename 之前 fsync（失败即中止），但 rename 之后对父目录的 fsync 会自行吞掉失败而不让调用失败，因为部分平台和文件系统（Windows、若干网络挂载）即便 rename 本身成功，仍会拒绝目录读取或同步。在这些情况下，或目录项 fsync 因其他原因失败时，即便内容本身已持久写入，崩溃后仍可能观察不到该次 rename。此处的文件型存储在启动时重新读取并重新发布，把这段残余的崩溃窗口留作调用方的策略。
