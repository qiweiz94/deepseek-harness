...
- **Single-session deletion is available, not automatic retention** — this backend registers a [`dsh-session-retention`](../session-retention) participant that removes the session-owned directory; age- or quota-based pruning is not wired and remains a deferred policy layered on that seam.
