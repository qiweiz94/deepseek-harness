...
- **Single-session deletion is available, not automatic retention** — this backend registers a [`dsh-session-retention`](../session-retention) participant that deletes the `sessions` row in one transaction and lets `ON DELETE CASCADE` remove its events; age- or quota-based pruning is not wired and remains a deferred policy layered on that seam.
