...
- **单会话删除已可用，但不是自动保留策略**：该后端注册了一个 [`dsh-session-retention`](../../session/session-retention) 参与者，用于移除被删除会话自身的 spill 目录。它不感知世系关系：被 fork 或恢复的会话可能仍在引用某个已删除父会话目录下的路径；基于时间或配额的保留策略尚未接入。
