...
- **该 seam 自身没有取回或删除 API**：消费方只能渲染后端的定位信息与指引；生命周期和访问语义仍由后端自行决定。后端可以另行向 [`dsh-session-retention`](../../session/session-retention) 注册以支持会话删除（`dsh-spill-local` 就是这样做的）；删除能力归属于该 seam，而非 `SpillStore`。
