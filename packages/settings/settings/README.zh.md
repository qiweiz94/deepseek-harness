...
- `register(ns, schema, { base?, applies?, validate? })` — 返回 owner 的 `SettingsScope`（`get`/`watch`/`update`）。注册是调用方插件 fiber 上的 effect：dispose（资源释放）该 fiber 即移除 namespace 及其观察者，并等待每个已启动的观察者调用结算完毕，因此没有回调能活过注册方。schema 拒绝的存量分节会使注册本身失败；重复 namespace 立即报错。
...
- 服务卸载先拒绝新写入与观察者调用的启动，再排干全部排队写入与已启动的观察者调用后才完成。registrant fiber 在写入途中被 dispose 时，该写入仍到达存储；它不会提交，也不会通知任何人——除非此间已挂载了替代注册：替代注册会依据已持久化的分节重新解析，而不是停留在陈旧值上（其 schema 或 `validate` 拒绝该分节时，替代注册保留最后可用值并告警）。
...
- **`redactSecrets` 对无法遍历之处 fail-closed，但序列化封装尚未净化**：walker 跟随 `object`/`dict`/`array`；只能经由 union、intersection、transform 或 tuple 节点抵达的 `role('secret')` 会使脱敏直接抛错，而不是原样返回值；声明了机密的容器形状无法套用到已存在的存量值上时，该值会被剥离并记录其容器位置。仍然暂缓：`schema.toJSON()` 会把 secret 字段的 `.default(...)` 一并带给每个客户端，跨 wire 的提供方错误文本也未净化——拥有封装与错误净化职责的 `describeForWire()` 仍是完整答案。
