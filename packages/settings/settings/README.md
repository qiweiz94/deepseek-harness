...
- `register(ns, schema, { base?, applies?, validate? })` — returns the owner `SettingsScope` (`get`/`watch`/`update`). The registration is an effect on the calling plugin's fiber: disposing that fiber removes the namespace and its observers, and waits until every started watcher invocation settled so no callback outlives the registrant. A stored section the schema rejects fails the registration itself; a duplicate namespace fails loud.
...
- Service teardown refuses new writes and watcher starts, then drains every queued write and every started watcher invocation before disposal completes. A write whose registrant fiber was disposed mid-flight still reaches storage; it commits and notifies nobody — unless a replacement registration mounted meanwhile, which re-resolves from the persisted section instead of staying stale (a section its schema or `validate` rejects keeps the replacement's last good value and warns).
...
- **`redactSecrets` fails closed on what it cannot walk, but the serialized envelope is not sanitized** — the walker follows `object`/`dict`/`array`; a `role('secret')` reachable only through a union, intersection, transform, or tuple node makes redaction throw instead of returning the value verbatim, and a present stored value that a secret-declaring container shape cannot be applied to is stripped with its container position recorded. Still deferred: `schema.toJSON()` carries a secret field's `.default(...)` to every client, and provider error text crossing the wire is not sanitized — a `describeForWire()` owning envelope and error sanitization remains the full answer.
