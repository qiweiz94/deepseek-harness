...
- **父级可能在宿主启动 dispose 后继续接受报告**：`AgentHandle.dispose()` 会先取消并等待完全停稳，然后才撤销作用域并离开注册表；它不公开「dispose 已开始」信号。在该窗口内接受的报告会在尝试发送之前先持久记录到父级会话日志中，因此即便销毁流程的取消丢弃或从未真正投递这次排队发送，该父级也会在其下一次恢复时向自己重新投递该报告，而不是丢失它。对于由继续执行管理器拥有的父级，管理器的准入边界会在整片森林拆卸期间拒绝该上报。
- **接受是至少一次而非恰好一次投递**：现在有一个持久化 mailbox 能在父级重启与有序销毁后仍然存活（参见 `dsh-subagent` 的「上报投递是至少一次而非恰好一次」），但仍然没有幂等键、投递回执，也不保证恰好一次。在认领轮次自身的 `user/message` 写入丢失的窄窗口内仍可能重复；外部重试也可能产生重复上报。
