...
- **A parent whose host-owned disposal already started can still accept** — `AgentHandle.dispose()` cancels, awaits quiescence, and only then unwinds the scope and leaves the registry; it exposes no signal for "disposal started." A report accepted in that window is durably recorded in the parent's session log before the send is attempted, so even when disposal's cancel drops or never delivers the queued send, that parent redelivers the report to itself on its next resume rather than losing it. A continuation-manager-owned parent rejects forest teardown through the manager's admission boundary.
- **Acceptance is at-least-once, not exactly-once, delivery** — a durable mailbox now survives parent restart and orderly teardown (`dsh-subagent`'s "Report delivery is at-least-once, not exactly-once"), but there is still no idempotency key, delivery receipt, or exactly-once claim. Duplicates remain possible in the narrow window where the claiming turn's own `user/message` write is lost, and an external retry may still duplicate the report.
