...
- **上报投递是至少一次而非恰好一次**：已接受的上报会在收件箱发送之前持久记录到父级的会话日志中（`subagent/report`）；未投递的记录会在父级下次恢复时重新投递，直到认领它的轮次将其写为 `user/message` 为止。在认领轮次自身的 `user/message` 写入丢失的窄窗口内仍可能重复，并且依然没有回给上报 child 的已读回执，没有宿主用户接收方，也没有跨进程投递——该 mailbox 存在于父级自身进程本地的会话日志中。
