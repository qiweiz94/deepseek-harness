 ...
 * Per-activation knobs such as `maxTokens` and `reasoningEffort` are omitted
 * for the same reason as `outputSchema`: they budget one activation. Cold
 * resume requires the exact live parent for authorization but reconstructs
 * child options only from the durable descriptor, so it neither restores the
 * prior budget/effort nor inherits the parent's current one; the resumed
 * route's defaults apply instead.
 *
