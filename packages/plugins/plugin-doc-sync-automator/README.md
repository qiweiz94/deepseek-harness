...
The model sees the `sync_bilingual_pair` schema: a required `docPath` string and a required `updatedSection` object carrying a `heading` string. Plugin config (repository root, budget-manifest path, tool name) is validated at load; it changes no schema field, only where the tool reads and writes.
