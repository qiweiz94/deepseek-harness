...
The model sees one optional `files` array of repo-relative paths. Plugin config (repository root, tsconfig, suite patterns, runner command, suite and byte bounds, timeout) is validated at load and fails loud on invalid values (e.g. a non-positive `maxOutputBytes`); it changes no schema field, only which suites exist and how they run.
