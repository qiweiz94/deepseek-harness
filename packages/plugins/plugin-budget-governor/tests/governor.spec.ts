...
describe('resolveSettings', () => {
  ...
  it('rejects a zero token budget rather than breaching on the first call', () => {
    // oxlint-disable-next-line typescript/no-misused-spread -- bypasses schema validation to reach resolveSettings' bound check
  ...
  it('skips every churn bound when churn accounting is disabled', () => {
    const settings = resolveSettings(governorConfig({
      churn: { enabled: false, tools: [], pathKeys: [], contentKeys: [], repeatThreshold: 0 },
    }))
