...
- **Trusted caller boundary** — `ctx.sessionQuery` performs no per-caller authorization of its own; it is a same-process, context-wide read/search primitive, and each consumer that exposes it (or its results) beyond its own process authorizes access at its own boundary before doing so. [`dsh-tool-session-query`](../tool-session-query/README.md)'s model-facing tools authorize by exact caller-session `cwd` equality; [`dsh-host-apiproxy`](../../host/apiproxy/README.md)'s `session.search` RPC filters provider hits against Host session-list visibility before returning them; [`dsh-session-reference`](../../context/session-reference/README.md) documents that it assumes its host is already authorized to read every session it can reach through this service. A future consumer that crosses a wire, model, or multi-tenant boundary owns that same authorization step.
